...
var instrucciones = ["Tene en cuenta que, cuando muevas las piezas, siempre quedara libre la pieza vacía", "Move una a una las piezas en la dirección correcta", "Forma el rompecabezas como lo indica la imagen de prueba"];
...
function chequearSiGano() {
  for (var i = 0; i < grilla.length; i++) {
    for (var m = 0; m < grilla.length; m++) {
      if (grilla[i][m] !== grillaGanadora[i][m]) {
        return false;
      }
    }
  }
...
function mostrarCartelGanador() {
  if (chequearSiGano) {
    alert('Has ganado el Rompecabezas');
  }
}
